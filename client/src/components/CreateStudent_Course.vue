    ...
    methods: {
      ...
      async enrollInCourse() {
        ...
        const res = await axios.post(
          'http://localhost:3001/api/student_course/create-student-course',
          {
            score: score,
            letter: this.grade,
            student_name: this.student_name,
            course_name: this.courses[this.mathNum].name,
      ...
      async DeleteStudent() {
        await axios.delete(
          `http://localhost:3001/api/student/delete-student/${this.student_id}`
        )
